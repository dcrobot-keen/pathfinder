...
//                             지금 버전에서는 장애물로 변환하지 않는다 -- "바깥쪽이
//                             막힘"을 표현하려면 큰 사각형에서 room을 뺀 홀(hole)
//                             폴리곤이 필요한데, Go 쪽 RasterizeBlocks가 홀을 실제로
//                             지원하는지 검증되지 않았다. 대신 그대로 room feature로
//                             출력해 두어(카테고리 유지) 나중에 뷰 초기 범위 힌트나
//                             2단계 벽-장애물화 작업에 쓸 수 있게 한다.
//
...
function parseArgs(argv) {
  const positional = [];
  const options = { wallThickness: DEFAULT_WALL_THICKNESS_M, room: null };
...
function convertFeature(feature, { room, wallThickness, importedAt }) {
  ...
  if (category === 'room') {
    ...
    return {
      ...
      note: 'room 폴리곤은 장애물(block)로 변환하지 않고 참고용(kind: room-outline)으로만 보존됨',
    };
  }
