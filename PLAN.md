...
3. **베이스맵 좌표계 규약**: 원점/기준축을 어떻게 정의할지 (예: 스캔 시작점, 특정 마커, 바닥 중심 등)
...
6. **뷰어 기술 선택**: gltf-inspector 코드 재사용 vs 신규 three.js 뷰어 구축
...
| 이동 물체 제거 | ❌ 미구현 | 스캔 중 지나간 사람 등 동적 객체 제거 — 실제 iPhone 스캔에서 필요해질 가능성 높음 |
| 삼각망 구축/구멍메우기/평활화 | ❌ 미구현 | 포인트클라우드 대신 깔끔한 메시로 베이스맵을 만들고 싶을 때 |
| 자동/수동 분류(바닥/벽/가구) | ❌ 미구현 | 있으면 로봇 지도 정합 정확도·시각화 품질 향상, 우선순위 낮음 |
