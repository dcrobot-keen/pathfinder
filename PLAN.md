...
**정합 UI — 자동 ICP + 수동 확인/미세조정 (`studio/align_viewer_html.py`)**: "정합을 UI로 어떻게 풀지" 논의 결과, 완전자동 ICP만으로는 위험하다고 판단해 채택한 방향. 실내 지도는 로봇 탐색 범위와 LiDAR 스캔 범위가 부분적으로만 겹치고, 복도/방이 반복되는 구조가 많아 ICP가 국소적으로 그럴듯하지만 전역적으로 틀린 정렬에 빠질 수 있는데, RMSE/겹침 수치만으로는 이걸 못 잡아냄 — 사람이 눈으로 보면 바로 알 수 있음.
- **UI**: 자동 ICP 결과를 초기값으로 self-contained HTML(서버 불필요, 기존 뷰어들과 같은 아키텍처)에 베이스맵(회색, 고정)과 로봇맵(빨강)을 캔버스로 오버레이. **드래그**로 평행이동, **Shift+드래그**로 로봇맵 자기 자신의 중심 기준 제자리 회전(원점 기준으로 돌리면 화면 밖으로 튕겨나가서, 중심을 고정하고 그 둘레로 도는 방식을 채택). 베이스맵의 공간해시로 실시간 최근접거리/겹침% 표시, 각도·이동량 숫자 직접 입력도 가능, "변환값 내보내기"로 최종 회전/이동을 JSON 다운로드.
- **연동**: `scripts/register_maps.py --align-html align.html`, `scripts/studio.py process --robot-map ...`가 자동으로 `projects/<name>/align.html` 생성 + report.html에 링크.
- **검증**: Chrome에서 실제로 띄워서 드래그(이동)/Shift+드래그(회전)/숫자입력/리셋/내보내기 버튼까지 전부 동작 확인. 회전이 고정된 피벗(파란 점) 중심으로 정확히 도는 것도 확인.
...
**접근 재전환 (사용자 피드백)**: 위 Hough 선분 방식으로 만든 GeoJSON을 본 사용자가 "GeoJSON을 잘 못 만드는구나.. 다른 접근이 필요하겠다"고 지적. 원인을 다시 보니 근본적 한계였음 — **Hough는 독립된 선분들만 찾을 뿐 서로 이어진다는 보장이 전혀 없음**. 그래서 방 하나도 제대로 닫힌 도형이 안 되고(벽 7개가 서로 다 떨어져 있음), 일부 방은 벽 자체가 검출 안 됨.

**새 방식 — 윤곽선(contour) 추출로 전환**: 개별 벽을 하나씩 찾아 이어붙이는(상향식) 대신, **분류 라벨과 무관하게 스캔된 모든 포인트**를 2D로 래스터화해 "스캔이 된 영역 전체"의 이진 마스크를 만들고, `cv2.findContours`로 그 바깥 경계를 추출(하향식). 윤곽선은 정의상 항상 닫힌 도형이므로 이전 방식의 근본 문제(비연결성)가 구조적으로 해결됨. 이후 작은 스캔 틈(가구에 가려진 부분 등)은 모폴로지 close로 메우고, `cv2.approxPolyDP`로 계단 모양 픽셀 경계를 직선 폴리곤으로 단순화. 가구도 같은 방식(컴포넌트별 컨투어 + 단순화)으로 바꿔 기존 바운딩박스보다 실제 모양에 가깝고 인접 가구가 하나로 뭉개지는 문제도 해결.
- 새 함수: `rasterize_interior_mask`, `trace_room_polygons`, `detect_furniture_polygons` (`studio/vectorize.py`). 기존 `detect_wall_lines`/`merge_collinear_segments`는 `--walls` 플래그로 보조/디버그 출력으로 남김.
- **검증**: 합성 데이터(4m×5m 방) → 정확히 4개 꼭짓점, 면적 20.03m²(실제 20.00m²) 폴리곤 1개로 정확히 복원 (`tests/test_vectorize.py`, PASS). 실제 오피스 스캔 → 방 폴리곤 1개(194.7m², 다중 공간 건물 전체 실루엣과 시각적으로 일치)로 닫힌 도형 생성 확인 — 이전 방식(7개 끊긴 선분)과 질적으로 다른 결과.
- **참고**: 이 방식은 "탐색된 영역의 외곽선"이라 실제 벽 두께/정확한 벽 위치보다는 "로봇이 갈 수 있는 공간의 경계"에 더 가까움 — 로봇 지도 정합 목적에는 이게 더 적합할 수 있음(occupancy grid의 free space 경계와 개념이 같음).

**직각 스냅(orthogonal rectify) 추가**: 컨투어로 뽑은 폴리곤은 여전히 픽셀 노이즈로 들쭉날쭉함(실제 스캔에서 꼭짓점 100개 이상). 실제 건물은 거의 항상 직각 그리드 위에 지어져 있다는 점(스캔 좌표계 자체가 회전돼 있어도 마찬가지)에 착안해 `rectify_orthogonal()` 추가.
  - **각도 검출**: 폴리곤 변들의 각도를 길이로 가중해 90도 mod로 히스토그램을 만들고 최빈값을 건물의 대표 각도로 사용(`dominant_angle_deg`) — 짧은 노이즈 변이 많아도 긴 실제 벽 변들이 투표를 지배해서 안정적.
  - **스냅**: 그 각도만큼 회전시킨 좌표계에서 각 변을 수평/수직으로 분류하고(더 가까운 쪽), 각 꼭짓점을 인접한 두 변(이제 상수 x 또는 상수 y인 직선)의 **교점**으로 재계산. 꼭짓점을 순서대로 앞으로 밀어내는 방식(forward propagation)은 폴리곤이 원래 자리로 닫힐 때 오차가 누적되는 문제가 있어서, 교점 방식(polygon 전체에 걸쳐 대칭적이라 인덱스 순서와 무관하게 정확히 닫힘)을 채택.
  - **사전 단순화 강도 상향**: 직각 스냅만으로는 원래 컨투어의 자잘한 요철(계단 모양)까지 다 90도로 꺾인 계단으로 남아서 안 예뻐짐 → `trace_room_polygons`의 기본 `simplify_epsilon_m`을 0.12m → 0.35m, `close_radius_m`을 0.2m → 0.3m로 올려서 작은 요철을 사전에 눌러버림. (작은 알코브/구석까지 보존하려면 `--simplify` 낮춰서 재실행 가능.)
...
| **점군 정렬/병합(registration)** | 🟡 구현됨(합성/대역 데이터만 검증) | 2D ICP + 다중 회전 시드 + 수동 확인/미세조정 UI(`align.html`, §Phase 3 "2D ICP 정합" / "정합 UI") — 알고리즘·UI 다 완성했지만 실제 로봇이 만든 occupancy grid로는 아직 한 번도 테스트 못 함 |
...
**결론 (갱신 2회)**: 점군/지도 정렬(registration)은 알고리즘(ICP)과 사람이 확인/보정하는 UI(align.html)까지 구현·검증(합성 + 오피스 대역 데이터) 완료됨 — 남은 건 구현이 아니라 **실제 로봇 occupancy grid 확보 후 실전 검증**(§6 질문 4, 회사 방문 필요). "이동 물체 제거"도 마찬가지로 알고리즘은 있지만 실제 사람이 지나간 스캔이 없어 검증 못 한 상태 — 두 항목 다 병목은 구현이 아니라 실제 데이터 확보.
